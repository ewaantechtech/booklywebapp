...
<body dir="rtl" style="direction: rtl; text-align: right;">
    <div style="direction: rtl; text-align: right;">
        ...
        <p style="text-align: right;">عزيزتي 
            @if($type == 'customer')
        ...
        <div class="section" style="direction: rtl; text-align: right;">
            ...
            <!-- --------------- --> 
        @if($type == 'customer' && $goodwill)
        @php
            $date = $appointment->services[0]->pivot->date ?? now();
            $serviceDate = Carbon\Carbon::parse($date);
            $messageText = 'Your booking #' . $appointment->id . ' with ' . $appointment->serviceProvider->name . ' on ' . $serviceDate->format('l') . ' & ' . $serviceDate->format('d-m-Y') . ' & ' .$appointment->services[0]->pivot->start_time . ' has been cancelled due to '. str_replace('_', ' ' , $appointment->admin_cancel_reason) . '. You will be refunded the full deposit amount of SAR ' . $appointment->total_payed . ' to your bank account within ' . config('app.refund_days') . ' days.
            Because you’re a valued customer, we have transferred SAR ' . $appointment->goodwill_amount  . ' to your app wallet to use for your next booking.';
          @endphp
           <div class="info-line" style="text-align: right;">
                <span class="label">تم الإلغاء :</span>
                <span class="value">{{ $messageText }}</span>
            </div>
        @endif
        @if($type == 'customer' && !($goodwill))
            @php
        ...
        @endif 
         @if($type == 'provider')
            ...
            $date = $appointment->services[0]->pivot->date ?? now();
            $serviceDate = Carbon\Carbon::parse($date);
            $messageText = 'Appointment #' . $appointment->id . ' of ' . $appointment->customer->first_name . " " . $appointment->customer->last_name . ' on ' . $serviceDate->format('l') . ' & ' . $serviceDate->format('d-m-Y') . ' & ' .$appointment->services[0]->pivot->start_time . ' has been cancelled due to '. str_replace("_", " "  , $appointment->admin_cancel_reason) . '.' ;
            @endphp
              <div class="info-line" style="text-align: right;">
                <span class="label">تم الإلغاء :</span>
                <span class="value">{{ $messageText }}</span>
